...
__global__ void averageImg(int *out, int *img, int width, int height) {
  ...
  int r = 0, g = 0, b = 0, n = 0;
  for (int l = blockIdx.x - 1; l < blockIdx.x + 2 && l < height; l++)
    for (int c = threadIdx.x - 1; c < threadIdx.x + 2 && c < width; c++)
      if (l >= 0 && c >= 0) {
        int idx = 3 * (l * width + c);
        r += img[idx];
        g += img[idx + 1];
  ...
  int idx = 3 * (blockIdx.x * width + threadIdx.x);
  out[idx] = r / n;
  out[idx + 1] = g / n;
  out[idx + 2] = b / n;
}
...
int main(int argc, char *argv[]) {
  ...
  clock_t t = clock();
  cudaMalloc((int **)&d_img, 3 * imgw * imgh * sizeof(int));
  cudaMalloc((int **)&d_out, 3 * imgw * imgh * sizeof(int));
  cudaMemcpy(d_img, img, 3 * imgw * imgh * sizeof(int), cudaMemcpyHostToDevice);
  averageImg<<<imgh, imgw>>>(d_out, d_img, imgw, imgh);
  cudaMemcpy(out, d_out, 3 * imgw * imgh * sizeof(int), cudaMemcpyDeviceToHost);
  cudaFree(d_img);
  cudaFree(d_out);
  t = clock() - t;
