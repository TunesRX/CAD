 ...
 */
void averageImg(int*out, int*img, int line, int col, int width, int height) {
	int r=0,g=0,b=0, n=0;
...
__global__ void averageImgCuda(int*out, int*img, int width, int height){
    int r=0,g=0,b=0, n=0;
    int line = blockIdx.x;
    int col = threadIdx.x;
    for (int l=line-1; l<line+2 && l<height; l++)
...
int main(int argc, char *argv[]) {
	...
	averageImgCuda<<<imgh,imgw>>>(d_b,d_a, imgw, imgh);
