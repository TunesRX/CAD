 ...
 * (some changes by Vitor Duarte Oct 2021)
 */
 ...
 */
double pi_cpu(unsigned long niter) {
	double pi, x;
	const double h = 1.0 / (double) niter;
...
int main(int argc, char *argv[]) {
	...
	unsigned long niter, chunks, workGroups;
	...
	commandQueue = clCreateCommandQueueWithProperties(context, device, NULL, &clError);
	...
	/************** Read output buffer **************/
	double pi = 0;
	double *pi_partial = new double[workGroups];
	clError = clEnqueueReadBuffer(commandQueue, memObjects[0], CL_TRUE, 0,
			sizeof(double)*workGroups, pi_partial, 0, NULL, NULL);
	...
	gettimeofday(&t0, NULL);
	double pi_cpu_value = pi_cpu(niter);
	gettimeofday(&t1, NULL);
	...
	std::cout << std::setprecision(5) << "GPU = " << t_gpu.tv_sec*1000+t_gpu.tv_usec/1000.0 << "ms"
			<< std::endl;
	std::cout << std::setprecision(5) << "CPU = " << t_cpu.tv_sec*1000+t_cpu.tv_usec/1000.0 << "ms"
			<< std::endl;
